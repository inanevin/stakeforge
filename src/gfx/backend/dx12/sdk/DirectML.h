...
STDAPI DMLCreateDevice(ID3D12Device*		   d3d12Device,
...
STDAPI DMLCreateDevice1(ID3D12Device*			d3d12Device,
						DML_CREATE_DEVICE_FLAGS flags,
...
{
	IFACEMETHOD(GetPrivateData)(REFGUID guid, _Inout_ UINT * dataSize, _Out_writes_bytes_opt_(*dataSize) void* data) = 0;

	IFACEMETHOD(SetPrivateData)(REFGUID guid, UINT dataSize, _In_reads_bytes_opt_(dataSize) const void* data) = 0;

	IFACEMETHOD(SetPrivateDataInterface)(REFGUID guid, _In_opt_ IUnknown * data) = 0;

	IFACEMETHOD(SetName)(PCWSTR name) = 0;
};
...
{
	IFACEMETHOD(CheckFeatureSupport)(DML_FEATURE feature, UINT featureQueryDataSize, _In_reads_bytes_opt_(featureQueryDataSize) const void* featureQueryData, UINT featureSupportDataSize, _Out_writes_bytes_(featureSupportDataSize) void* featureSupportData) = 0;

	IFACEMETHOD(CreateOperator)
	(const DML_OPERATOR_DESC* desc,
	 REFIID					  riid, // expected: IDMLOperator
	 _COM_Outptr_opt_ void**  ppv) = 0;
	...
	IFACEMETHOD(Evict)(UINT count, _In_reads_(count) IDMLPageable* const* ppObjects) = 0;

	IFACEMETHOD(MakeResident)(UINT count, _In_reads_(count) IDMLPageable* const* ppObjects) = 0;

	IFACEMETHOD(GetDeviceRemovedReason)() = 0;

	IFACEMETHOD(GetParentDevice)(REFIID riid, _COM_Outptr_ void** ppv) = 0;
};
...
{
	IFACEMETHOD(Reset)(UINT operatorCount, _In_reads_opt_(operatorCount) IDMLCompiledOperator* const* operators) = 0;
};
...
{
	IFACEMETHOD_(void, BindInputs)(UINT bindingCount, _In_reads_opt_(bindingCount) const DML_BINDING_DESC* bindings) = 0;

	IFACEMETHOD_(void, BindOutputs)(UINT bindingCount, _In_reads_opt_(bindingCount) const DML_BINDING_DESC* bindings) = 0;

	IFACEMETHOD_(void, BindTemporaryResource)(_In_opt_ const DML_BINDING_DESC* binding) = 0;

	IFACEMETHOD_(void, BindPersistentResource)(_In_opt_ const DML_BINDING_DESC* binding) = 0;

	IFACEMETHOD(Reset)(_In_opt_ const DML_BINDING_TABLE_DESC* desc) = 0;
};
...
{
	IFACEMETHOD_(void, RecordDispatch)(ID3D12CommandList * commandList, IDMLDispatchable * dispatchable, IDMLBindingTable * bindings) = 0;
};
...
{
	IFACEMETHOD_(void, SetMuteDebugOutput)(BOOL mute) = 0;
};
