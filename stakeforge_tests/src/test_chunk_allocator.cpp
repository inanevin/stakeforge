...
namespace sfg
{
	namespace tests
	{
		namespace
		{
			bool growth_preserves_offset_handles()
			{
				test_context_t context;
				context.suite	 = "chunk_allocator";
				context.name	 = "growth_preserves_offset_handles";
				context.failures = 0;
				...
				const u32			   old_capacity = allocator.get_capacity();
				const chunk_handle32_t second		= allocator.allocate_bytes(64, alignof(u32));

				SFG_TEST_EXPECT(context, allocator.get_capacity() > old_capacity);
		...
		{
			register_test("chunk_allocator", "growth_preserves_offset_handles", &growth_preserves_offset_handles);
