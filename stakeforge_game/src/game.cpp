...
#define GAME_RAW_WHEEL_DELTA 120.0f
	...
	void game_t::lock_script_cursor(script_cursor_lock_mode_e mode)
	{
		...
		if (mode == script_cursor_lock_mode_e::center)
			...
			process::set_cursor_confinement(window.window_handle, mode == script_cursor_lock_mode_e::current_position ? window_cursor_confinement_e::pointer : window_cursor_confinement_e::none);
	}
