...
namespace sfg
{
	namespace
	{
		...
		bool write_component_to_stream(const nlohmann::json& component, ostream_t& stream)
		{
			...
			if (!reflection_registry_t::get().deserialize_from_json(type_id, data, component.value("data", nlohmann::json::object())))
			{
				SFG_ALIGNED_FREE(data);
				SFG_ERR("failed to deserialize prefab component: {0}", type_id);
				return false;
			}
			if (!reflection_registry_t::get().serialize_to_stream(type_id, data, stream))
			{
				SFG_ALIGNED_FREE(data);
		...
		{
			stream << entity.value<entity_guid_t>("guid", NULL_ENTITY_GUID);
			stream << entity.value<string_t>("name", "");
