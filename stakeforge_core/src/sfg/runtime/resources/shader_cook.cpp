...
namespace sfg
{
	...
	bool shader_cooker::cook_from_file(const shader_cook_config_t& cfg, const char* full_path, resource_header_t& out_header, ostream_t& stream, shader_data_definition_t& out_definition)
	{
		...
		case shader_type_e::debug_triangle:
			...
			break;
		case shader_type_e::debug_texture:
			if (!shader_cook_variants_t::cook_debug_texture_shader(compile_source, include_paths, compiles, psos))
			{
				SFG_ERR("failed to cook debug texture shader variants: {0}", full_path);
				return false;
			}
			break;
