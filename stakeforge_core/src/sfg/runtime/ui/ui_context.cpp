...
#define DEBUG_HOVER_RECT_TEXT_RESERVE 64
	...
	void ui_context::tick(const vec4f_t& screen_rect, f32 dpi_scale, f32 dt_seconds)
	{
		...
		_tree.set_topology_mutation_allowed(true);
		_input.begin_frame(_tree, dt_seconds);
		run_pre_layout_ticks(dt_seconds);
		_input.prepare_layout(_tree);

		_tree.set_topology_mutation_allowed(false);
